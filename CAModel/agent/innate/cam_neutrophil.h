#ifndef __CAM_NEUTROPHIL_H__
#define __CAM_NEUTROPHIL_H__
...
int cam_neutrophil_set_params (YAML::Node* cfg);

/* enum of Neutrophil Type */
enum class NeutrophilType
{
  null = 0
};


/* enum of Neutrophil State */
enum class NeutrophilState
{
  null = -1,
  resting = 0,
  active = 1,
...
private:
  ...
  /* neutrophil type */
  NeutrophilType neut_type;

  /* neutrophil state */
  NeutrophilState neut_state;

  /* neutrophil is resident on cell */
  ...
  void set_secretion (const NeutrophilState st);
  void regulate_secretion ();
  void update_secretions ();

  /* lifespan */
  void update_lifespan (const NeutrophilState st);

  /* set the uptake */
  void set_uptake (const NeutrophilState st);
  void regulate_uptake ();

  /* set the movement */
  void set_movement (const NeutrophilState st);
  void update_movement (const NeutrophilState st);

  int phagocytose_epithelial (const double time);
  void phagocytose_immune (const double time);

public:
    Neutrophil (const int x, const int y, const NeutrophilType tp, const NeutrophilState st,
                struct cam_rd_sys* rds, const bool use_output, int* status);
    ~Neutrophil ();

    /* neutrophil type */
    NeutrophilType get_type ();
    void set_type (const NeutrophilType tp);

    /* neutrophil cell state */
    NeutrophilState get_state ();
    void set_state (const NeutrophilState st);
    void change_state (const NeutrophilState st);
...
/* get the neutrophil type */
inline NeutrophilType
Neutrophil::get_type ()
{
  return neut_type;
}

/* set the neutrophil type */
inline void
Neutrophil::set_type (const NeutrophilType tp)
{
  neut_type = tp;
}

/* get the neutrophil state */
inline NeutrophilState
Neutrophil::get_state ()
{
  return neut_state;
}
...
Neutrophil::get_code ()
{
  return (static_cast<int>(Agent::get_code()) + static_cast<int>(neut_type) + static_cast<int>(neut_state));
}

/* set the neutrophil as resident on a cell */
inline void
Neutrophil::set_as_resident_on_cell (void* cell)
{
  resident_on_cell = (Epithelial*) cell;
}

/* get the cell the neutrophil is resident on  */
inline void*
Neutrophil::get_resident_on_cell ()
{
  return resident_on_cell;
}

inline void
Neutrophil::dec_rds_values ()
{
  /* coords */
...
inline bool
Neutrophil::is_inflammatory ()
{
  return false;
}

inline bool
Neutrophil::is_active ()
{
  return ( (neut_state == NeutrophilState::active) ? true : false );
}

/* check whether agent is apoptotic */
inline bool
Neutrophil::is_apoptotic ()
{
  return ( (neut_state == NeutrophilState::apoptotic) ? true : false );
}

inline bool
Neutrophil::is_phagocyte ()
{
  return true;
}

inline void
Neutrophil::write_output (const double time)
{
  if (output != nullptr) {
    /* coords */
    std::pair<int, int> xy = get_coordinates();
...
/* get internalised */
inline double
Neutrophil::get_internalised ()
{
  return total_virus_uptaken;
}

#endif /* __CAM_NEUTROPHIL_H__ */
